...
public class Search implements ISearch{
	...
	public List<PropertyEntry> doSearch(Location preferredLocation, double minPrice, double maxPrice, int minBedrooms,
		...
		*/
		...
		//6. minBathrooms,
		//7. maxBathrooms. 
		Payload searchPayload = new Payload(propertyIDsSearchSpace,preferredLocation, minPrice, maxPrice, minBedrooms, maxBedrooms, minBathrooms, maxBathrooms);
		
		//send it to propertyMatcher server. 
		List<PropertyEntry> result = filterPropertyAndFindMatchPercent(searchPayload);
		
		sortPropertyResult(result);
		
		return result;
	}

	private void sortPropertyResult(List<PropertyEntry> result) {
		//Since collections.sort uses merge sort, if we sort the data before merging in the slave servers itself, cumulative sort will be faster as the sub arrays will be already sorted. 
	...
	private List<PropertyEntry> SplitAndMergeMatchPercentFindingProcess(Payload searchPayload) {
		...
		//Abstracting that master server will compute the results in parallel and load balanced among its slave servers and aggregate the results. 
		
		return findMatch(searchPayload);
	}
	
	// will be running in master server.
	private List<PropertyEntry> findMatch(Payload searchPayload)
	{
		//... load balancing 
		//... threads. 
		
		// one portion of code running in its slave server will look like below
		List<PropertyEntry> propEntryList = new ArrayList<>();
		for(Long propId : searchPayload.getPropertyIDsSearchSpace())
		{
			PropertyEntry propEntry = findMatchPercentForProperty(propId, searchPayload.getSearchFilter());
			if(propEntry.getMatchPercent()>40)
			{
				propEntryList.add(propEntry);
			}
		}
		
		sortPropertyResult(propEntryList);
		
		return propEntryList;
		
	}
	 
	// in slave server. 
	private PropertyEntry findMatchPercentForProperty(Long propertyId, SearchFilter filter)
	{
	...
	private Property getPropertyDetailsFromDB(Long propertyId) {
		return new Property();
	}
